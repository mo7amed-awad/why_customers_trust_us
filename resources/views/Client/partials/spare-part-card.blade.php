<div class="card property-card p-1 ">
    ...
    <div class="card-body row gap-2">
        ...
        <div class=" col-12 d-flex gap-2 flex-wrap justify-content-between">
            <span class="bg-Secondary-color p-2 rounded-2 text-dark price-badge text-muted fs-12">{{ $item->sparePartDetails->brand->trans('title') ?? 'N/A' }}</span>
            <span class="bg-Secondary-color p-2 rounded-2 text-dark price-badge text-muted fs-12">{{ $item->sparePartDetails->carModel->trans('title') ?? 'N/A' }}</span>
        ...
        <div class="d-flex justify-content-end align-items-center mb-2 col-12">
            <a class="btn primary-color d-flex align-items-center gap-2" href="{{route('client.ad',['id' => $item->id ])}}"><span>{{ __('front.details') }}</span><span><i class="fa-solid fa-arrow-right arrow"></i></span></a>
        </div>
