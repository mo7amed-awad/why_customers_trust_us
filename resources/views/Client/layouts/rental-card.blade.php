<div class="card border-0 car-card" data-aos="zoom-in">
    <div class="card-body p-0">
        <div class="car-img-height image-container rounded-2">
            <img src="{{ asset($Rental->Image?->image ?? setting('logo')) }}" class="img-fluid w-100 h-100 object-fit-cover rounded-2" alt="{{ $Rental->title() }}">
        </div>
        <div class="mt-4">
            <div>
                <h5 class="text-success fw-semibold">{{ $Rental->title() }}</h5>
                <div class="d-flex flex-wrap justify-content-between mb-3">
                    @foreach ($Rental->Specifications->take(4) as $Specification)
                ...
                <h5 class="text-success fw-semibold">BHD {{ format_number($Rental->price) }}</h5>
            </div>
            <div>
                <a href="{{ route('client.rental.checkout',$Rental) }}" class="btn btn-primary rounded-3 w-100">{{ __('trans.Book Now') }}</a>
            </div>
        </div>
    </div>
</div>