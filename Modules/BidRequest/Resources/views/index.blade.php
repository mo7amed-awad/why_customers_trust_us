...
@section('content')
    ...
    <div class="row">
        @if (hasPermission('add_bid_requests'))
            <div class="my-2 col-6 text-sm-start">
                <a href="{{ route(activeGuard() . '.bid_requests.create') }}" class="main-btn">@lang('trans.add_new')</a>
            </div>
        @endif
    </div>
...
@push('js')
    <script>
        var table = $('#DataTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.datatable-bid_requests') }}',
                data: function(d) {
