...
@section('content')
    ...
    <div class="row">
        @if (hasPermission('add_rentals'))
            <div class="my-2 col-6 text-sm-start">
                <a href="{{ route(activeGuard() . '.rentals.create') }}" class="main-btn">@lang('trans.add_new')</a>
            </div>
        @endif
    </div>
...
@push('js')
    <script>
        var table = $('#DataTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.datatable-rentals') }}',
                data: function(d) {
