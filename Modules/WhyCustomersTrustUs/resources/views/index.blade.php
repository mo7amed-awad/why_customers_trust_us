...
@section('content')
    ...
    <div class="row">
        @if (hasPermission('add_whycustomerstrustus'))
            <div class="my-2 col-6 text-sm-start">
                <a href="{{ route(activeGuard() . '.whycustomerstrustus.create') }}" class="main-btn">@lang('trans.add_new')</a>
            </div>
        @endif
    </div>
...
@push('js')
    <script>
        var table = $('#DataTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.datatable-whycustomerstrustus') }}',
                data: function(d) {
