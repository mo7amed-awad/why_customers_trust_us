...
@section('content')
    ...
    <div class="row">
        @if (hasPermission('add_biddings'))
            <div class="my-2 col-6 text-sm-start">
                <a href="{{ route(activeGuard() . '.biddings.create') }}" class="main-btn">@lang('trans.add_new')</a>
            </div>
        @endif
    </div>
    <table class="table table-bordered dataTable table-striped" id="DataTable">
        <thead>
            <tr>
                <th>#</th>
                <th>@lang('trans.brand')</th>
                <th>@lang('trans.model')</th>
...
@push('js')
    <script>
        var table = $('#DataTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.datatable-biddings') }}',
                data: function(d) {
