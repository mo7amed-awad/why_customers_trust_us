...
@section('pagetitle', __('trans.biddings'))

@section('content')
    <form method="POST" action="{{ route(activeGuard() . '.biddings.update', $Model) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="row">
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2" required id="model_id" name="model_id">
                    <option value=""></option>
                    @if($Model->brand_id)
                        @foreach ($Brands->where('id', $Model->brand_id)->first()->Models ?? [] as $model)
                            <option value="{{ $model->id }}" {{ $Model['model_id'] == $model->id ? 'selected' : '' }}>
                                {{ $model->title() }}
                            </option>
                        @endforeach
                    @endif
            ...
            <div class="form-group col-md-6">
                <label for="bid_time">@lang('trans.bid_time')</label>
                <input type="datetime-local" name="bid_time" id="bid_time" class="form-control" required value="{{ $Model['bid_time'] }}">
            </div>
            ...
            <div class="col-12 my-4">
                <div class="specs-card">
                    ...
                    <div id="specifications-wrapper">
                        @foreach ($Model->Specifications as $index => $spec)
                            <div class="specification-item d-flex gap-3 align-items-center">
                                <div class="spec-item-number">#{{ $index + 1 }}</div>
                                <div class="spec-controls">
                                    <button type="button" class="btn spec-move-up">⬆️</button>
                                    <button type="button" class="btn spec-move-down">⬇️</button>
                                </div>
                                <div class="custom-select-wrapper">
                                    <input type="text" class="form-control spec-search" value="{{ $spec->title() }}" placeholder="🔍 @lang('trans.search')..." autocomplete="off">
                                    <span class="search-icon">▼</span>
                                    <input type="hidden" name="specifications[{{ $spec->id }}][id]" class="spec-value" value="{{ $spec->id }}">
                                    <div class="spec-dropdown" style="display:none;">
                                        @foreach ($Specifications as $s)
                                            <div class="spec-option" data-value="{{ $s->id }}" {{ $spec->id == $s->id ? 'data-selected="true"' : '' }}>
                                                {{ $s->title_en }}
                                            </div>
                                        @endforeach
                                    </div>
                                ...
                                <div class="spec-inputs-group">
                                    <input type="text" name="specifications[{{ $spec->id }}][title_ar]" class="form-control" placeholder="📝 @lang('trans.title_ar')" value="{{ $spec->pivot->title_ar }}">
                                    <input type="text" name="specifications[{{ $spec->id }}][title_en]" class="form-control" placeholder="📝 @lang('trans.title_en')" value="{{ $spec->pivot->title_en }}">
                                </div>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th style="text-align:center;">@lang('trans.actions')</th>
                </tr>
            </thead>
            <tbody id="image-table-body">
                @foreach ($Model->images as $key => $image)
                    <tr class="sortable-row" data-id="{{ $image['id'] }}">
                        <td style="text-align:center;">{{ $key + 1 }}</td>
                        <td style="text-align:center;">
                            <img src="{{ $image['image'] }}" alt="{{ $Model['title_en'] }}" width="150">
                        </td>
                        <td style="text-align:center;">
                            ...
                            <i class="fa-solid fa-trash-can cursor-pointer" data-id="{{ $image['id'] }}" data-table="bidding_images"></i>
                        </td>
                    </tr>
                @endforeach
            </tbody>
...
@push('js')
    <script>
        ...
        function updateOrderOnServer() {
            const order = [];
            $('#image-table-body .sortable-row').each(function() {
                order.push($(this).data('id'));
            });

            $.ajax({
                url: '{{ route('admin.rentals.images.sort', $Model->id) }}',
                type: 'POST',
                data: {
        ...
        $(document).on('click', '.move-up', function() {
            ...
            const prev = row.prev('.sortable-row');
            if (prev.length) {
        ...
        $(document).on('click', '.move-down', function() {
            const row = $(this).closest('tr');
            const next = row.next('.sortable-row');
            if (next.length) {
        ...
        $(document).on('click', '.fa-trash-can', function() {
            ...
            $('#image-table-body tr').each(function(index) {
