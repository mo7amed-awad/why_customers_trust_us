...
@section('pagetitle', __('trans.faqs'))
@section('content')


    <div class="row">
        @if (hasPermission('add_faqs'))
            <div class="my-2 col-6 text-sm-start">
                <a href="{{ route(activeGuard() . '.faqs.create') }}" class="main-btn">@lang('trans.add_new')</a>
            </div>
        @endif
    </div>
...
@push('js')
    <script>
        var table = $('#DataTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.datatable-faqs') }}',
                data: function(d) {
